...
from utils.constants import (DEFAULT_IMAGE_TOKEN,
                             ...
                             DEFAULT_VIDEO_TOKEN,
                             VIDEO_TOKEN_INDEX,
                             ONE_PLACEHOLDER_PER_IMAGE,
                             ONE_PLACEHOLDER_PER_VIDEO)
...
class VideoLLMProcessor(object):
    ...
    def __init__(self,
                 ...
                 dummy_frame_shape = (0, 3, 384, 384),
                 max_batch_frames: int = 16,
                 num_segments: int = 8,
                 training: bool = True,
                 verbose: bool = True,
                 task_type: str = "completion",
                 vison_placeholder_mode: int = ONE_PLACEHOLDER_PER_IMAGE,
        ...
        assert padding_side in (
            "left", "right"), f"unsupport padding_size: {padding_side}"
        ...
        self.label_key = [label_key] if isinstance(
            label_key, str) else label_key
        ...
        self.vison_placeholder_mode = vison_placeholder_mode
        self.vision_placeholder = DEFAULT_IMAGE_TOKEN \
                                if self.vison_placeholder_mode == ONE_PLACEHOLDER_PER_IMAGE else DEFAULT_VIDEO_TOKEN
        self.vision_placeholder_index = IMAGE_TOKEN_INDEX \
                        if self.vison_placeholder_mode == ONE_PLACEHOLDER_PER_IMAGE else VIDEO_TOKEN_INDEX
        self.shuffle_vqa = shuffle_vqa
        self.vqa_processor = VQAProcessor(
            self.label_key[0], self.vision_placeholder, task_type=task_type, **vqa_processor_params)
        ...
        if self.with_visual:
            if self.frames_key in data_dict:
                ...
                if isinstance(frames[0], (str, os.PathLike)):
                    ...
                    # frames = [load_local_frame(frame) for frame in frames]
                    with ThreadPoolExecutor(max_workers=32) as executor:
                        frames = [frame[0] for frame in executor.map(
                ...
                data_dict["image_size"] = list(self.dummy_frame_shape)[-2:]
                data_dict["n_frames"] = 0
                # data_dict[self.frames_key] = []

            # --------------set data_mode-----------------
            if self.task_type in ["vqa", "dpo"]:
                ...
                if vqa_key in data_dict:
                    ...
                    if self.shuffle_vqa and len(data_dict.get(self.frames_key, [])) > 1:
                        n_per_group = 2 if self.task_type == "vqa" else 3
                        group_indices = list(range(0, len(data_dict[vqa_key]), n_per_group))
    ...
    def transform(self, data_dict):
        ...
        if 'input_ids' in output and self.vison_placeholder_mode == ONE_PLACEHOLDER_PER_IMAGE:
            ...
            if self.frames_key in data_dict and self.with_visual:
                if cur_image_num != len(data_dict[self.frames_key]):
                    print(f"mismatching size: {cur_image_num}, {len(data_dict[self.frames_key])}")
    ...
    def collate_frames(self, batch_data, collate_data):
        ...
        for data in batch_data:
            if self.frames_key in data:
                frame_len_list.append(len(data[self.frames_key]))
                frames_list.append(data.pop(self.frames_key))
            else:
    ...
    def padding_sequence(self, inputs, value):
        ...
        for t in inputs:
            if len(t) < max_length:
                ...
                pad = (0, pad_len) if self.padding_side == "right" else (
                    pad_len, 0)
    ...
    def batch_transform(self, batch_data):
        ...
        collate_data['vison_placeholder_mode'] = self.vison_placeholder_mode
        collate_data['vision_placeholder'] = self.vision_placeholder
        return collate_data

    def batch_transform_for_dpo(self, batch_data):
        # FIXME might merge with batch_transform
        collate_data = {}
        ...
        for key in all_keys:
            if key.endswith("_input_ids"):
                tmp_value = [data.pop(key) for data in batch_data]
                tmp_value = self.padding_sequence(tmp_value, value=self.pad_id)
                collate_data[key] = tmp_value
            elif key.endswith("_labels"):
                tmp_value = [data.pop(key) for data in batch_data]
                tmp_value = self.padding_sequence(
                    tmp_value, value=self.ignore_index)
                collate_data[key] = tmp_value
            elif key.endswith("_attention_mask"):
                tmp_value = [data.pop(key) for data in batch_data]
                tmp_value = self.padding_sequence(tmp_value, value=0)
                collate_data[key] = tmp_value
            # FIXME Do we need left padding for prompt-related terms?
    ...
    def add_vision_placeholders_in_prompt(self, data_dict):
        ...
        """
        def add_timestamp(frame_count, frame_prefix_pattern="{i}s: ", offset=1, remove_single_frame_timestamp=True, sep="; ", remove_last_sep=False, end_symbol="\n"):
                ...
                if self.task_type in data_dict:
                    ...
                    for item in data_dict[self.task_type]:
                        ...
                        if DEFAULT_VIDEO_TOKEN in item["value"]:
                            ...
                            break
                    if not exist_flag:
                        data_dict[self.task_type][0]["value"] = image_mode_prompt \
                            + data_dict[self.task_type][0]["value"]
            else:
                image_mode_prompt = add_timestamp(
                        len(data_dict[self.frames_key]), **self.timestamp_params)
                data_dict["input_prompt_template"] = data_dict["input_prompt_template"].replace(
                    DEFAULT_VIDEO_TOKEN, image_mode_prompt)
        else:
            # in video mode, in video mode: replace <image> with <video> directly
            if self.task_type in ["vqa", "dpo"]:
                if self.task_type in data_dict:
                    for item in data_dict[self.task_type]:
                        if DEFAULT_IMAGE_TOKEN in item["value"]:
                            item["value"] = item["value"].replace(
    ...
    def sample_frames(self, data_dict):
        ...
        elif self.sample_method == "every_n_frame":
            interval = 2
            part_frames = frames
            # if not self.training:
            if len(part_frames) / interval >= self.max_batch_frames:
                frames_index = random_index(
                    len(part_frames), self.max_batch_frames, average_draw)
                part_frames = [part_frames[i] for i in frames_index]
            else:
                part_frames = part_frames[::interval]
        elif self.sample_method == "random_clip":
            total_frames = len(data_dict[self.frames_key])
            if self.training:
                clip_len = random.randint(
                    self.clip_frames[0], self.clip_frames[1])
                interval = random.randint(
                    self.clip_interval[0], self.clip_interval[1])
                if clip_len * interval >= total_frames:
                    interval = 1
                    start = 0
                else:
                    start = random.randint(
                        0, total_frames - clip_len * interval)

                part_frames = frames[start:start +
                                     clip_len * interval:interval]
                for key in self.extra_sample_keys:
                    assert len(
                        data_dict[key]) == total_frames, f"Mismatch between {key}_len: {len(data_dict[key])} and frame_len {total_frames}!"
                    data_dict[key] = data_dict[key][start:start +
                                                    clip_len * interval:interval]
            else:
    ...
    def build_text(self, data_dict):
        if self.task_type == "dpo":
            prompt_list, chosen_response_list, rejected_response_list = self.vqa_processor(
                data_dict)
            prompt_ids_list = [
                self.tokenizer_vision_placeholder(prompt, )
                for prompt in prompt_list
            ]

            chosen_ids_list = [
                self.tokenizer.encode(response, )
                for response in chosen_response_list
            ]
            chosen_input_ids, chosen_label_mask = [], []

            rejected_ids_list = [
                self.tokenizer.encode(response, )
                for response in rejected_response_list
            ]
            rejected_input_ids, rejected_label_mask = [], []

            # FIXME no truncate now
            for prompt_id, chosen_id, rejected_id in zip(prompt_ids_list, chosen_ids_list, rejected_ids_list):
                chosen_input_ids += prompt_id + chosen_id
                chosen_label_mask += [0] * \
                    len(prompt_id) + [1] * len(chosen_id)
                rejected_input_ids += prompt_id + rejected_id
                rejected_label_mask += [0] * \
                    len(prompt_id) + [1] * len(rejected_id)

                chosen_input_ids = chosen_input_ids + [self.eos_id]
                chosen_label_mask = chosen_label_mask + [1]
                rejected_input_ids = rejected_input_ids + [self.eos_id]
                rejected_label_mask = rejected_label_mask + [1]

            chosen_input_mask = [1] * len(chosen_input_ids)
            chosen_input_ids = torch.as_tensor(
                chosen_input_ids, dtype=torch.int64)
            chosen_label_mask = torch.as_tensor(
                chosen_label_mask, dtype=torch.int64)
            chosen_attention_mask = torch.as_tensor(
                chosen_input_mask, dtype=torch.int64)
            chosen_label = chosen_input_ids.masked_fill(
                chosen_label_mask != 1, self.ignore_index)

            rejected_input_mask = [1] * len(rejected_input_ids)
            rejected_input_ids = torch.as_tensor(
                rejected_input_ids, dtype=torch.int64)
            rejected_label_mask = torch.as_tensor(
                rejected_label_mask, dtype=torch.int64)
            rejected_attention_mask = torch.as_tensor(
                rejected_input_mask, dtype=torch.int64)
            rejected_label = rejected_input_ids.masked_fill(
                rejected_label_mask != 1, self.ignore_index)
            ...
            if self.label_key[0] in data_dict:
                ...
                prompt_list, response_list = self.online_vqa_processor(
                    data_dict)
            prompt_ids_list = [
                self.tokenizer_vision_placeholder(prompt) for i, prompt in enumerate(prompt_list)
            ]
            ...
            for i, (prompt_id, response_id) in enumerate(zip(prompt_ids_list, response_ids_list)):
                if len(input_ids) + len(prompt_id) + len(response_id) >= self.max_seq_len - 1 and self.truncate_mode == "qa" and i > 0:  # truncating conversation instead of truncating a sentence
                    print(f"Warning! Get incoming text length {len(input_ids) + len(prompt_id) + len(response_id)} >= max length {self.max_seq_len}. Truncate qa now.")
                    print(data_dict["image_folder"])
                    break
                elif len(input_ids) + len(prompt_id) + len(response_id) >= self.max_seq_len - 1 and (self.truncate_mode == "text" or i == 0):
                    print(f"Warning! Get incoming text length {len(input_ids) + len(prompt_id) + len(response_id)} >= max length {self.max_seq_len}. Truncate text now.")
                    print(data_dict["image_folder"])
...
class VideoLLMPredictProcessor(VideoLLMProcessor):
    ...
    def build_text(self, data_dict):
        ...
        ret["vid"] = data_dict.get("vid", data_dict.get("task_id", ""))
        # ret["id"] = data_dict["id"]
